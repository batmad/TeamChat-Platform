...
} from "@/lib/widget-auth/chat-session";
...
export async function requireChatSession(request: Request) {
  ...
  if (!token)
    throw new AppError(
      ...
      "Chat session token is required",
    );
  ...
  if (!payload)
    throw new AppError(
      ...
      "Chat session token is invalid or expired",
    );
  ...
  if (
    ...
    throw new AppError(
      ...
      "Chat session is no longer authorized",
    );
  }
  ...
  if (!application)
    throw new AppError(
      401,
      "CHAT_APPLICATION_INVALID",
      "Chat application is unavailable",
    );
