...
import {
  APPLICATION_RETENTION_DEFINITIONS,
...
const retentionSchema = z.object({
  policies: z
    .array(
      z
        .object({
          dataType: z.enum(["LOG", "CHAT", "ATTACHMENT"]),
        ...
        .superRefine((value, ctx) => {
          if (value.keepForever && value.retentionDays !== null) {
            ctx.addIssue({
              code: "custom",
              message: "retentionDays must be null when keepForever is true",
            });
          }
          if (!value.keepForever && value.retentionDays === null) {
            ctx.addIssue({
              code: "custom",
              message: "retentionDays is required when keepForever is false",
            });
          }
        }),
    )
...
function validDefinition(
  ...
  return APPLICATION_RETENTION_DEFINITIONS.find(
    (item) => item.dataType === dataType && item.category === category,
  );
}
...
export const PUT = withApiHandler(async (request, context: Context) => {
  ...
  const uniqueKeys = new Set(
    body.policies.map((item) => `${item.dataType}:${item.category}`),
  );
  if (uniqueKeys.size !== body.policies.length) {
    throw new AppError(
      400,
      "DUPLICATE_RETENTION_POLICY",
      "Each retention category can only be configured once",
    );
  }
  ...
  for (const policy of body.policies) {
    if (!validDefinition(policy.dataType, policy.category)) {
      throw new AppError(
        ...
        `Unsupported retention category: ${policy.dataType}:${policy.category}`,
      );
    }
  }

  const before = await prisma.retentionPolicy.findMany({
    where: { applicationId, isActive: true },
  });
