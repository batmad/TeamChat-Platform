...
import { readAttachmentForPreview } from "@/lib/attachments/message/read";
...
const handledGet = withApiHandler(async (request: Request, context: Context) => {
  ...
  const delivery = await readAttachmentForPreview({
    attachmentId,
  ...
  return new Response(Buffer.from(delivery.bytes), {
    ...
    headers: {
      "Content-Type":
        delivery.access.detectedMimeType || delivery.access.mimeType,
