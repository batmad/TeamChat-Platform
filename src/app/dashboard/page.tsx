...
export default async function DashboardPage() {
  ...
  if (!sessionHasPermission(session, "dashboard.view")) {
    const firstAllowed = getAllowedDashboardMenu(session).find(
      ...
      <div className="mx-auto max-w-7xl">
        <div className="rounded-3xl bg-white p-8 shadow-sm">
          ...
          <p className="mt-4 max-w-3xl text-slate-600">
            Login aktif sebagai <strong>{session.name}</strong>. Role dan
            permission efektif selalu dihitung ulang dari database pada request
            server.
          </p>
          ...
          <dl className="mt-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <div className="rounded-2xl bg-slate-50 p-5">
              ...
              <dd className="mt-1 font-semibold text-slate-950">
                {session.isRoot ? "Global" : session.applicationName}
              </dd>
            </div>
            ...
            <div className="rounded-2xl bg-slate-50 p-5">
              ...
              <dd className="mt-1 font-semibold text-slate-950">
                {session.isRoot ? "ROOT" : (session.role?.name ?? "No role")}
              </dd>
            </div>
            ...
            <div className="rounded-2xl bg-slate-50 p-5">
              ...
              <dd className="mt-1 font-semibold text-slate-950">
                {session.isRoot ? "Full access" : session.permissions.length}
              </dd>
            </div>
          </dl>
          ...
          {session.isRoot && (
            <div className="mt-8 border-t border-slate-200 pt-6">
              <a
                href={`${APP_BASE_PATH}/widget-demo.html`}
                target="_blank"
